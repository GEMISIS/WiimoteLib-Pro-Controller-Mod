...
namespace WiimoteLib
{
	...
	public class Wiimote : IDisposable
	{
        ...
        // VID = Nintendo, PID = Wiimote, Wii U Pro Controller PID = 0x330
		private const int VID = 0x057e;
		private const int PID = 0x0306;
        private const int WUP_PID = 0x0330;
			...
			while(HIDImports.SetupDiEnumDeviceInterfaces(hDevInfo, IntPtr.Zero, ref guid, index, ref diData))
			{
				...
				if(HIDImports.SetupDiGetDeviceInterfaceDetail(hDevInfo, ref diData, ref diDetail, size, out size, IntPtr.Zero))
				{
					...
					if(HIDImports.HidD_GetAttributes(mHandle.DangerousGetHandle(), ref attrib))
					{
                        ...
                        if (attrib.VendorID == VID && (attrib.ProductID == PID || attrib.ProductID == WUP_PID))
						{
							// it's a Wiimote
		...
		private void ParseExtension(byte[] buff, int offset)
		{
            switch (mWiimoteState.ExtensionType)
            {
                ...
                case ExtensionType.BalanceBoard:
                    ...
                    break;
                case ExtensionType.WiiUPro:
                    mWiimoteState.WiiUPro.A = (buff[12] & 0x10) == 0;
                    mWiimoteState.WiiUPro.B = (buff[12] & 0x40) == 0;
                    mWiimoteState.WiiUPro.X = (buff[12] & 0x08) == 0;
                    mWiimoteState.WiiUPro.Y = (buff[12] & 0x20) == 0;
                    mWiimoteState.WiiUPro.Minus = (buff[2] & 0x10) != 0;
                    mWiimoteState.WiiUPro.Home = (buff[2] & 0x80) != 0;
                    mWiimoteState.WiiUPro.Plus = (buff[1] & 0x10) != 0;
                    mWiimoteState.WiiUPro.Up = (buff[12] & 0x01) == 0;
                    mWiimoteState.WiiUPro.Down = (buff[11] & 0x40) == 0;
                    mWiimoteState.WiiUPro.Left = (buff[12] & 0x02) == 0;
                    mWiimoteState.WiiUPro.Right = (buff[11] & 0x80) == 0;
                    mWiimoteState.WiiUPro.L = (buff[11] & 0x20) == 0;
                    mWiimoteState.WiiUPro.R = (buff[11] & 0x02) == 0;
                    mWiimoteState.WiiUPro.LZ = (buff[12] & 0x80) == 0;
                    mWiimoteState.WiiUPro.RZ = (buff[12] & 0x04) == 0;
                    mWiimoteState.WiiUPro.LS = (buff[13] & 0x02) == 0;
                    mWiimoteState.WiiUPro.RS = (buff[13] & 0x01) == 0;

                    mWiimoteState.WiiUPro.RawJoystickLeft.X = (buff[4] << 8) | (buff[3]);
                    mWiimoteState.WiiUPro.RawJoystickLeft.Y = (buff[8] << 8) | (buff[7]);
                    mWiimoteState.WiiUPro.RawJoystickRight.X = (buff[6] << 8) | (buff[5]);
                    mWiimoteState.WiiUPro.RawJoystickRight.Y = (buff[10] << 8) | (buff[9]);

                    mWiimoteState.WiiUPro.JoystickLeft.X = (mWiimoteState.WiiUPro.RawJoystickLeft.X - 1900) / 100;
                    mWiimoteState.WiiUPro.JoystickLeft.Y = (mWiimoteState.WiiUPro.RawJoystickLeft.Y - 2100) / 100;
                    mWiimoteState.WiiUPro.JoystickRight.X = (mWiimoteState.WiiUPro.RawJoystickRight.X - 2100) / 100;
                    mWiimoteState.WiiUPro.JoystickRight.Y = (mWiimoteState.WiiUPro.RawJoystickRight.Y - 2100) / 100;
                    break;
            }
		}
